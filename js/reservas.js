class Reservas{
    ...
    async seleccionarRecurso(recurso) {
        ...
        const respuesta = await fetch(`php/horarios.php?id_recurso=${recurso.id_recurso}`);
        ...
        labelFecha.innerHTML = `
                Fecha:
                <input type="date" name="fecha" value="${recurso.fecha}" readonly/>
            `;
        ...
        form.onsubmit = async (e) => {
            ...
            if(window.confirm("La reserva se guardará. ¿Desea continuar?"))
                this.añadirReserva(recurso, personas, horario, presupuesto);
        };
    }

    async añadirReserva(recurso, personas, horario, presupuesto) {
        ...
        this.finalizarReserva(recurso.nombre, recurso.fecha, horario, personas, presupuesto);
    }
