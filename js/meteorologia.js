        ...
        $.ajax(
        {
            ...
            success: function(data) 
            {
                ...
                for(let i = 0; i < 7; i++) {
                    ...
                    const lista = $('<ul>').append(
                        $('<li>').text(`Temperatura media: ${temp}${tempUnit}`),
                        $('<li>').text(`Probabilidad de lluvia: ${precipitaciones}${precipitacionesUnit}`),
                        $('<li>').text(`Velocidad media del viento: ${viento}${vientoUnit}`)
                    );
                    desplegable.append(lista);
