<?php
    ...
    class Reserva extends Teverga {
        ...
        public function añadirReserva() {
            ...
            // Comprobar si ya existe una reserva del usuario con el mismo recurso y horario
            $sql = "SELECT id_reserva, personas, presupuesto
                    FROM reservas
                    WHERE id_usuario = ? AND id_horario = ?";
            $stmt = $this->conn->prepare($sql);
            $stmt->bind_param("ii", $this->id_usuario, $this->id_horario);
            $stmt->execute();
            ...
            if ($reservaExistente = $result->fetch_assoc()) { // Actualizar la reserva
                ...
                $stmt->close();
                $sql = "INSERT INTO reservas (id_usuario, id_horario, presupuesto, personas) VALUES (?, ?, ?, ?)";
                $stmt = $this->conn->prepare($sql);
                $stmt->bind_param("iidi", $this->id_usuario, $this->id_horario, $this->presupuesto, $this->personas);
                $stmt->execute();
                $stmt->close();
            }

            $this->actualizarPlazas(-$this->personas);
        }

        public function verReservas() {
            $this->connect();

            $sql = "SELECT r.id_reserva, rc.id_recurso, rc.fecha, r.personas, r.presupuesto, h.hora_inicio, h.hora_fin, rc.nombre
                    FROM reservas r
        ...
        public function actualizarPlazas($plazas) {
            $this->connect();

            $sql = "UPDATE recursos SET plazas = plazas + ? WHERE id_recurso = ?";
            $stmt = $this->conn->prepare($sql);
            $stmt->bind_param("ii", $plazas, $this->id_recurso);
